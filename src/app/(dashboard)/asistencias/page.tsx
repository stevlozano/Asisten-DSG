...
export default function Page(){
  ...
  return <div className="space-y-6">
    ...
    <div className="flex gap-2"><button title="Filtra para ver solo registros de hoy (oculta faltas)" onClick={()=>setFilterToday(!filterToday)} className={`px-5 py-2.5 rounded-full text-sm flex items-center gap-2 ${filterToday?"bg-[#1F9A75] text-white":"bg-black dark:bg-white text-white dark:text-black"}`}><Clock className="h-4 w-4"/> {filterToday?"Ver todos":"Solo hoy: sin faltas"}</button><button onClick={handleExport} className="px-5 py-2.5 rounded-full border bg-zinc-100 dark:bg-zinc-900 text-sm flex items-center gap-2"><FileText className="h-4 w-4"/> Exportar</button></div></div>
    <div className="bg-zinc-100 dark:bg-zinc-900 border dark:border-white/10 rounded-2xl p-4 text-sm text-black/60 space-y-1"><div className="flex items-center gap-2"><Clock className="h-4 w-4"/> Regla: la asistencia genera incidencia, no descuento automático.</div><p className="text-xs">Botón "Solo hoy: sin faltas" oculta las faltas para que veas solo quién asistió hoy. Útil para el parte diario del admin.</p></div>
